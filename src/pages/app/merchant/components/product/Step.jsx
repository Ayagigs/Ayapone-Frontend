...
const Step = ({
  ...
  const { setActiveStep } = useProductCreateStore()
  
  return (
    <>
      <li className='grid place-items-center w-full' onClick={() => setActiveStep(index)}>
        ...
        <div className={`text-center text-sm max-w-[60px] ${ (isActive || isPast) ? 'font-bold' : 'font-normal' }`}>{ title }</div>
      </li>
    </>
  )
}
